...
function connectCharacter(ws) {
    try {
        ...
        const idAccount = pkg.getString().trim();
        const idCharacter = pkg.getString().trim();
        const email = pkg.getString().trim();
        const typeGame = pkg.getByte();
        const idChar = pkg.getByte();

        login.connect(ws, idAccount, idCharacter, email, typeGame, idChar);
    } catch (err) {
        funct.dumpError(err);
    }
}
            ...
            if (dmg !== 0) {
                ...
                game.loopArea(ws, function(client) {
                    if (!client.isNpc) {
                        ...
                        handleProtocol.playSound(tileSelected.id, datSpell.wav, vars.clients[client.id])
